...
| Endpoint | Purpose |
|---|---|
| `POST /v1/handshake` | Protocol + capability report (`{runner: {ok}, tools: [{name, ok}]}`); required before claiming (426 on major-version mismatch). Claims 412 when a handler has agent stages and the runner is not ok, or a required tool is missing |
...
| `POST /v1/results` | Submit a stage/output result (see invariants); optional `artifacts: [paths]` records pointers into the case home's artifacts lane — content stays opaque to the platform |
...
| `GET /v1/items/:id` | Full case record: `meta`, `content` pointer, state, `generation`, latest results (corrections winning), `artifacts` pointer union |
